...
router.get('/api/snippets', requireAuth, (req, res) => {
    res.redirect('/api/snippets' + (req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : ''));
});

router.get('/api/snippets/public', (req, res) => {
    res.redirect('/api/snippets/public' + (req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : ''));
});
