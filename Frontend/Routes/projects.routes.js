...
router.get('/api/projects', requireAuth, (req, res) => {
    res.redirect('/api/projects' + (req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : ''));
});

router.get('/api/projects/public', (req, res) => {
    res.redirect('/api/projects/public' + (req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : ''));
});
