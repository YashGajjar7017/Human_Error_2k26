...
function updateAnalyticsStats(data) {
    ...
    document.getElementById('successRate').textContent = data.successRate ? data.successRate + '%' : '0%';
    document.getElementById('avgExecutionTime').textContent = data.avgExecutionTime ? data.avgExecutionTime + 'ms' : '0ms';
...
function updateRecentCompilations(compilations) {
    ...
    if (!compilations || compilations.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="5" class="text-center">No compilation history available</td></tr>';
        return;
    }

    tableBody.innerHTML = compilations.map(compilation => `
        <tr>
...
function updateInsights(data) {
    ...
    if (data.insights) {
        document.getElementById('improvementTrend').textContent = data.insights.improvementTrend || 'Keep coding to see your trends!';
        document.getElementById('mostUsedLanguage').textContent = data.insights.mostUsedLanguage || 'No data available';
        document.getElementById('peakHours').textContent = data.insights.peakHours || 'No data available';
        document.getElementById('achievementProgress').textContent = data.insights.achievementProgress || 'Start coding to unlock achievements!';
    }
